...
const steps = [
  ...
  },
  {
    ...
    desc: "For the Project, we'll schedule your kickoff call within a week. For Essentials, we'll begin with a comprehensive planning session.",
  },
];
      ...
      <div className="flex flex-col justify-center h-full px-20 py-16">
          ...
          {steps.map((step, i) => (
            <div key={i} className="flex-1 flex flex-col items-start">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-14 h-14 rounded-xl flex items-center justify-center" style={{ background: 'hsl(174, 30%, 93%)' }}>
                  <step.icon className="w-6 h-6" style={{ color: 'hsl(180, 38%, 30%)' }} />
                </div>
                <span className="text-5xl font-bold" style={{ color: 'hsl(174, 42%, 80%)' }}>{step.number}</span>
              </div>
        ...
        <div className="border-t border-slide-gray-200 pt-8 mt-auto">
          ...
          <p className="text-lg font-semibold mt-4" style={{ color: 'hsl(180, 38%, 30%)' }}>
            Chris Haigh, CFP® — <span className="font-normal text-slide-gray-500">CEO, Iconoclastic Capital Management</span>
          </p>
        </div>
      </div>
    </SlideLayout>
